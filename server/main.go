...
// startHeartbeatChecker 心跳检测增加写超时，防止 Write 阻塞
...
func addFriendApply(fromUser, toUser string) (string, error) {
	...
	sendSysMsg(
		toUser,
		fmt.Sprintf(
			"📩 收到好友申请：【%s】想加你好友，输入 pendinglist 查看",
			fromUser,
		),
	)
	...
	for {
					...
					if err != nil {
						...
						if retMsg == "✅ 消息撤回成功" {
							...
							if errScan == nil {
								...
								// 推送给接收方
								if recvConn :=
									getOnlineConn(receiver); recvConn != nil {

									_, _ = recvConn.Write(
										Encode([]byte(notifyText)),
									)
								}

								// 推送给撤回操作者本人
								if selfConn :=
									getOnlineConn(username); selfConn != nil {
		...
		case strings.HasPrefix(
			...
			if err != nil {
				...
				targetConn :=
					getOnlineConn(targetName)

				if targetConn == nil {

					resp = fmt.Sprintf(
						"💤【%s】离线，消息已保存，上线自动推送, msgId:%d",
						targetName,
						msgId,
					)
					...
					_, writeErr := targetConn.Write(
						Encode([]byte(privateMsg)),
					)
...
func handleConn(conn net.Conn) {
	...
	for {
		...
		if len(parts) >= 1 {
			...
			// 修复的关键 Bug：
			//
			// 客户端在登录界面每3秒发送一次 ping。
			// 原来的服务端没有处理 ping，
			// 会把 ping 当成非法的登录指令，
			// 返回：
			// ❌ 格式：register|user|pass...
			//
			// 客户端于是可能把这条错误消息当成认证响应。
			// ==================================================
			case "ping":
			...
			case "resetpwd":
				...
				if len(parts) != 4 {
					...
					resp =
						"❌ 格式 resetpwd|用户名|密保答案|新密码"

				} else {

					msg, err :=
						resetPasswordBySecAnswer(
							parts[1],
							parts[2],
							parts[3],
						)
		...
		if len(parts) != 3 {
			...
			resp =
				"❌ 格式：register|user|pass 或 login|user|pass，找回密码可用 getsecq|xxx / resetpwd|user|答案|新密码"
